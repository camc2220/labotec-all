...
public class ResultsController : ControllerBase
{
    ...
    public async Task<ActionResult<LabResultReadDto>> Create([FromBody] LabResultCreateDto dto)
    {
        ...
        var result = new LabResultReadDto(
            entity.Id,
            patient.Id,
            patient.FullName,
            entity.TestName,
    ...
    public async Task<IActionResult> GetPdfUrl(Guid id, [FromServices] IStorageService storage, [FromQuery] int minutes = 30)
    {
        ...
        // ✅ Staff puede acceder; paciente solo a lo suyo (y SOLO si trae patientId)
        if (!IsStaff())
        {
            if (!User.IsInRole("Paciente")) return Forbid();
            ...
            if (r.PatientId != patientId.Value) return Forbid();
        }
