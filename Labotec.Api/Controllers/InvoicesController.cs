...
public class InvoicesController : ControllerBase
{
    ...
    // ✅ (PUNTO 6) total calculado y precios protegidos
        ...
        bool allowInactive = false)
    {
        ...
        var requested = (itemsDto ?? Array.Empty<InvoiceItemCreateDto>())
            .Where(i => i.LabTestId != Guid.Empty)
            .ToList();

        if (!requested.Any())
            return (Array.Empty<InvoiceItem>(), null);

        // No permitir precios negativos (si vinieran)
        if (requested.Any(x => x.Price.HasValue && x.Price.Value < 0))
            return (Array.Empty<InvoiceItem>(), null);

        var labTestIds = requested.Select(i => i.LabTestId).Distinct().ToList();
        ...
        foreach (var dto in requested)
        {
            ...
            // ✅ Precio confiable SOLO desde BD
            var defaultPrice = test.DefaultPrice;
            if (!defaultPrice.HasValue)
                return (Array.Empty<InvoiceItem>(), null);

            // ✅ Si el cliente manda Price, tiene que ser igual al DefaultPrice (o null).
            if (dto.Price.HasValue && dto.Price.Value != defaultPrice.Value)
                return (Array.Empty<InvoiceItem>(), null);

            result.Add(new InvoiceItem
            {
                LabTestId = test.Id,
                Price = decimal.Round(defaultPrice.Value, 2, MidpointRounding.AwayFromZero)
            });
        }
    ...
    public async Task<ActionResult<InvoiceReadDto>> Create([FromBody] InvoiceCreateDto dto)
    {
        ...
        if (!items.Any()) return BadRequest("Alguna prueba no existe, está inactiva, no tiene precio o el precio fue alterado.");
    ...
    public async Task<IActionResult> Update(Guid id, [FromBody] InvoiceUpdateDto dto)
    {
        ...
        if (!items.Any()) return BadRequest("Alguna prueba no existe, no tiene precio o el precio fue alterado.");
        if (!total.HasValue) return BadRequest("Alguna prueba no existe o no tiene precio configurado.");
